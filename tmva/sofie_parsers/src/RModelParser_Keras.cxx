...
std::unique_ptr<ROperator>
MakeKerasBatchNorm(PyObject *fLayer); // For instantiating ROperator for Keras Batch Normalization Layer
...
std::unique_ptr<ROperator>
MakeKerasBinary(PyObject *fLayer); // For instantiating ROperator for Keras binary operations: Add, Subtract & Multiply.
...
std::unique_ptr<ROperator>
MakeKerasLeakyRelu(PyObject *fLayer);                           // For instantiating ROperator for Keras LeakyRelu Layer
   ...
   // For layers like Dense & Conv which has additional activation attribute
      ...
      if (fLayerActivation != "linear") {
         ...
         if (fLayerType == "Conv2D") {
            ...
            op_pre_transpose.reset(new ROperator_Transpose<float>(
               {0, 3, 1, 2}, PyStringAsString(PyList_GetItem(fInputs, 0)), fLayerName + "PreTrans"));
         ...
         if (fLayerType == "Conv2D") {
            ...
            op_post_transpose.reset(
               new ROperator_Transpose<float>({0, 2, 3, 1}, fLayerName + fLayerType, fLayerName + "PostTrans"));
         ...
         if (findActivationLayer == mapKerasLayer.end()) {
            throw std::runtime_error("TMVA::SOFIE - Parsing Keras Activation layer " + fLayerActivation +
...
std::unique_ptr<ROperator> MakeKerasDense(PyObject *fLayer)
{
   ...
   case ETensorType::FLOAT:
      op.reset(new ROperator_Gemm<float>(attr_alpha, attr_beta, attr_transA, attr_transB, fLayerInputName, fKernelName,
                                         fBiasName, fLayerOutputName));
      break;
...
std::unique_ptr<ROperator> MakeKerasConv(PyObject *fLayer)
{
   ...
   if (fKerasPadding == "valid") {
      ...
      long padding_height =
         std::max(long((outputHeight - 1) * fAttrStrides[0] + fAttrKernelShape[0] - inputHeight), 0L);
   ...
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT:
      op.reset(new ROperator_Conv<float>(fAttrAutopad, fAttrDilations, fAttrGroup, fAttrKernelShape, fAttrPads,
                                         fAttrStrides, fLayerInputName, fKernelName, fBiasName, fLayerOutputName));
      break;
...
std::unique_ptr<ROperator> MakeKerasReLU(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Relu<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
std::unique_ptr<ROperator> MakeKerasSelu(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Selu<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
std::unique_ptr<ROperator> MakeKerasSigmoid(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Sigmoid<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
std::unique_ptr<ROperator> MakeKerasLeakyRelu(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_LeakyRelu<float>(fAlpha, fLayerInputName, fLayerOutputName)); break;
   default:
      throw std::runtime_error("TMVA::SOFIE - Unsupported - Operator Sigmoid does not yet support input type " +
                               fLayerDType);
   }
...
std::unique_ptr<ROperator> MakeKerasTanh(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Tanh<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
std::unique_ptr<ROperator> MakeKerasSwish(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Swish<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
std::unique_ptr<ROperator> MakeKerasBatchNorm(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   op.reset(new ROperator_BatchNormalization<float>(fEpsilon, fMomentum, /* training mode */ 0, fNX, fNScale, fNB,
                                                    fNMean, fNVar, fNY));
   return op;
}
...
std::unique_ptr<ROperator> MakeKerasIdentity(PyObject *fLayer)
{
   ...
   std::unique_ptr<ROperator> op;
   switch (ConvertStringToType(fLayerDType)) {
   case ETensorType::FLOAT: op.reset(new ROperator_Identity<float>(fLayerInputName, fLayerOutputName)); break;
   default:
...
RModel Parse(std::string filename, int batch_size)
{
   ...
   PyRunString("for idx in range(len(model.layers)):\n"
               ...
               "  layerData['layerInput']=[x.name for x in layer.input] if isinstance(layer.input,list) else "
               "[layer.input.name]\n"
               "  layerData['layerOutput']=[x.name for x in layer.output] if isinstance(layer.output,list) else "
               "[layer.output.name]\n"
   ...
   PyRunString("for idx in range(len(model.get_weights())):\n"
               ...
               "  weightProp['value']=(model.get_weights())[idx].transpose((3,2,0,1)).copy() if ('conv' in "
               "model.weights[idx].name and model.weights[idx].shape.ndims == 4) else (model.get_weights())[idx]\n"
   ...
   for (Py_ssize_t weightIter = 0; weightIter < PyList_Size(fPWeight); weightIter++) {
      ...
      default:
         throw std::runtime_error("Type error: TMVA SOFIE does not yet weight data layer type" +
   ...
   PyRunString("inputShapes=model.input_shape if type(model.input_shape)==list else [model.input_shape]", fGlobalNS,
               fLocalNS);
   PyRunString("inputTypes=[]", fGlobalNS, fLocalNS);
   PyRunString("for idx in range(len(model.inputs)):\n"
               "  inputTypes.append(model.inputs[idx].dtype.__str__()[9:-2])",
               fGlobalNS, fLocalNS);
   ...
   if (PyTuple_Check(fPInputShapes)) {
      ...
      case ETensorType::FLOAT: {
         ...
         if (static_cast<int>(fInputShape[0]) <= 0) {
            ...
            if (batch_size <= 0)
               std::cout << " assume is 1 ";
            else
               std::cout << " use given value of " << batch_size;
      ...
      default:
         throw std::runtime_error("Type error: TMVA SOFIE does not yet support data type" +
   ...
   PyRunString("for layerName in model.output_names:\n"
               "    outputNames.append(model.get_layer(layerName).output.name)",
               fGlobalNS, fLocalNS);
